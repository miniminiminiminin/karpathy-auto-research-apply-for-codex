...
5. Use `plan.md` and `loop-status.md` to define the active slice.
6. Run planner -> executor -> evaluator -> memory loops.
7. Record each iteration in `iterations/` and summarize it in `results.tsv`.
8. Route to release only when the project is ready enough to stop iterating.
...
5. record a baseline proof path before changing `project/`
