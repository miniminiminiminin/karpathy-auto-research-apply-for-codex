...
- signal threshold matrix recorded when runtime monitoring matters:
- threshold quality reviewed:
- merge or pr option selected explicitly:
- review and release records consistent:
...
- sustainability materiality explicit:
- sustainability decision class explicit when material:
...
PASS IF
  Gates.implementation_review_complete = TRUE
  AND Gates.qa_review_complete = TRUE
  AND Gates.security_privacy_review_complete = TRUE
  AND Gates.workspace_isolation_verified = TRUE
  ...
  AND Gates.signal_threshold_matrix_recorded_when_runtime_monitoring_matters = TRUE
  AND Gates.threshold_quality_reviewed = TRUE
  AND Gates.review_and_release_records_consistent = TRUE
  AND approvals_verified = yes
  AND open_risk_profile_explicit = yes
  AND sustainability_materiality_explicit = yes
  AND sustainability_decision_class_explicit_when_material IS yes_when_material_or_not_applicable_with_reason
  AND sustainability_decision_quality_adequate = yes
  AND sustainability_decision_explicit_when_material IS yes_or_not_applicable

FAIL IF
  Gates.implementation_review_complete = FALSE
  OR Gates.qa_review_complete = FALSE
  OR Gates.security_privacy_review_complete = FALSE
  OR Gates.workspace_isolation_verified = FALSE
  ...
  OR Gates.signal_threshold_matrix_recorded_when_runtime_monitoring_matters = FALSE
  OR Gates.threshold_quality_reviewed = FALSE
  OR Gates.review_and_release_records_consistent = FALSE
  OR approvals_verified IS not_yes
  OR open_risk_profile_explicit IS not_yes
  OR sustainability_materiality_explicit IS not_yes
  OR sustainability_decision_class_explicit_when_material IS missing_or_no
  OR sustainability_decision_class_explicit_when_material = not_applicable AND sustainability_decision_not_applicable_reason IS missing
  OR sustainability_decision_quality_adequate IS not_yes
...
DecisionQuality.sustainability_decision_quality_adequate := yes ONLY IF
  decision_class_fits_materiality_and_evidence = yes
  AND mitigation_or_measurement_is_specific = yes_or_not_applicable
  AND decision_owner_is_named = yes
  AND review_time_is_named = yes
  AND trigger_or_follow_up_is_actionable_when_not_accept_now = yes_or_not_applicable
